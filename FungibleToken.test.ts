...
describe("token integration", () => {
  ...
  beforeAll(async () => {
    ...
    const { privateKey: tokenAdminKey, publicKey: tokenAdminAccount } = PrivateKey.randomKeypair()

    const { privateKey: newTokenAdminKey, publicKey: newTokenAdminAccount } = PrivateKey
      .randomKeypair()

    const { privateKey: tokenAKey, publicKey: tokenAAccount } = PrivateKey.randomKeypair()
    const tokenA = new FungibleToken(tokenAAccount)

    const { privateKey: tokenBKey, publicKey: tokenBAccount } = PrivateKey.randomKeypair()
    const tokenB = new FungibleToken(tokenBAccount)

    const { privateKey: thirdPartyKey, publicKey: thirdPartyAccount } = PrivateKey.randomKeypair()
    const thirdParty = new ThirdParty(thirdPartyAccount)

    const { privateKey: thirdParty2Key, publicKey: thirdParty2Account } = PrivateKey.randomKeypair()
  ...
  describe("deploy", () => {
    it("should deploy token contract A", async () => {
      ...
      tx.sign([context.deployer.privateKey, context.tokenAKey])
    ...
    it("should deploy token contract B", async () => {
      ...
      tx.sign([context.deployer.privateKey, context.tokenBKey])
    ...
    it("should deploy a third party contract", async () => {
      ...
      tx.sign([context.deployer.privateKey, context.thirdPartyKey, context.thirdParty2Key])
  ...
  describe("transfers", () => {
    ...
    it("should do a transaction constructed manually, approved by the token contract", async () => {
      ...
      await tx.sign([context.senderKey, context.deployer.privateKey]).prove()
      await tx.send()
